...
<div class="container-fluid">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                ...
                {else}
                <a id="pdf-{$con.uid|escape:'htmlall':'UTF-8'}" class="btn btn-info" role="button"  href="https://factura.com/api/publica/invoice/{$con.uid|escape:'htmlall':'UTF-8'}/pdf">Documento PDF</a>
                <a id="xml-{$con.uid|escape:'htmlall':'UTF-8'}" class="btn btn-default" href="https://factura.com/api/publica/invoice/{$con.uid|escape:'htmlall':'UTF-8'}/xml">Documento XML</a>
                {/if}
              </td>
              <td>
                {if $con.status eq 'cancelada'}
  ...
  function invoiceEmail(uid){
    ...
    $.ajax({
      ...
      success: function(response){
          if (response.status != 'error') {
            swal("¡Envío exitoso!", "Hemos enviado tu factura al correo electrónico", "success");
          }
      }
    });
  }
