 ...
 */
   ...
   event.preventDefault();
       ...
       if(selected_method == 04 || selected_method == 28 || selected_method == 03){
         if(num_cta_method == "" || num_cta_method.length < 4){
           ...
           Swal.fire("¡Algo ocurrió!", "Ingresa los últimos 4 dí­gitos de tu cuenta o tarjeta", "error");
           return false;
         }
 }
 ...
 if(selected_method == 0){
   ...
   $("#error").text('Selecciona un método de pago.');
   $("#alerts").show();
   return false;
 }else{
 ...
 function orderDetail(data){
  if ($('#contact-email').val() == '' || 
     ...
     Swal.fire({
       ...
       text: "El email, el RFC, la razon social, el régimen, o el CP no deben estar vacíos",
       icon: 'info',
